...
def build_random_data_pair():
    ...
    result[repr(Atomic("Float"))] = (float(np.random.normal(0,1)), float(np.random.normal(0,1)))
    result[repr(Atomic("Int"))] = (int(np.random.randint(-10,10)), int(np.random.randint(-10,10)))
    return result
...
def forward_looking_test(op_name):
    ...
    for pattern in patterns:
        ...
        if not np.array_equal(result_A, result_B, equal_nan=True):
            print("A\n", result_A[-3:, :5], "\nB\n", result_B[-3: :5])
            raise ValueError(f"Forward looking bias in {op_name}, {pattern}")

def test_forward_looking():
    op_names = STD_LIB.keys()
    for op_name in op_names:
        if op_name != "data":
            forward_looking_test(op_name)
