...
def parse_patterns(args):
    ...
    types = [(v.options if isinstance(v, Either) else [v])
...
def run_pattern(op, pattern, data):
    ...
    resultA = op(**paramsA).compute(cache=cache)
    resultB = op(**paramsB).compute(cache=cache)
...
if __name__ == "__main__":
    pytest.main()
    # TODO : calculated value should not change next day. (e.g. logic error in last day)
